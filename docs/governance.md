# 开源治理

## 项目治理模型

MxSec Platform 采用 **Maintainer 委员会制**，由核心维护者共同决策项目方向和重大技术选型。

### 角色定义

| 角色 | 职责 | 权限 |
|------|------|------|
| Maintainer | 项目整体方向、架构决策、版本发布、代码审查 | 合并 PR、管理 Issue、发布版本 |
| Committer | 特定模块的代码审查与合并 | 对应模块的 PR 合并权限 |
| Contributor | 提交代码、文档、Issue、讨论 | Fork + PR |

### 晋升路径

- **Contributor → Committer**: 持续贡献且代码质量稳定，经 Maintainer 提名并获多数同意
- **Committer → Maintainer**: 长期深度参与，对架构有全面理解，经全体 Maintainer 投票通过

## 决策流程

### 日常决策

Bug 修复、文档改进、小型功能增强等日常变更，由任一 Committer 审查通过后即可合并。

### 重大决策

涉及以下事项时，需提交设计文档并在 Issue 中公开讨论，经 Maintainer 多数同意后方可实施：

- 架构层面的重大变更
- 新增外部依赖或替换核心组件
- 不向后兼容的 API 变更
- 安全模型变更

## 版本管理

### 版本号规范

遵循 [Semantic Versioning 2.0.0](https://semver.org/)：

```
MAJOR.MINOR.PATCH
```

- **MAJOR**: 不兼容的 API 或协议变更
- **MINOR**: 向后兼容的功能新增
- **PATCH**: 向后兼容的 Bug 修复

### 发布流程

1. 确认所有计划功能已合并且测试通过
2. 更新版本号和 CHANGELOG
3. 创建 Release Tag
4. 构建并发布安装包和镜像
5. 更新文档

## 安全策略

### 漏洞报告

如果你发现安全漏洞，**请勿在公开 Issue 中披露**。请通过以下方式私密报告：

- 邮件：security@mxsec.io
- 主题格式：`[SECURITY] 简要描述`

我们承诺在收到报告后 48 小时内确认，并在 7 个工作日内给出初步评估。

### 漏洞处理流程

1. 收到报告后分配安全响应负责人
2. 确认漏洞真实性和影响范围
3. 开发修复方案（在私有分支中进行）
4. 发布安全更新和公告
5. 在 CHANGELOG 中记录（不泄露利用细节）

### 安全更新

安全修复将作为 PATCH 版本发布，并通过以下渠道通知：

- GitHub Security Advisory
- Release Notes

## 代码质量

### 审查标准

所有合并到主分支的代码必须满足：

- 至少一名 Committer 或 Maintainer 审查通过
- 通过 CI 流水线（lint + test）
- 不引入已知安全漏洞
- 遵循项目代码规范

### 测试要求

- 核心路径单元测试覆盖率 >= 85%
- 整体覆盖率 >= 70%
- 新增功能必须包含对应测试
- Bug 修复必须包含回归测试

## 行为准则

本项目遵循 [Contributor Covenant 2.1](https://www.contributor-covenant.org/version/2/1/code_of_conduct/) 行为准则。

参与本项目即表示你同意遵守该准则。违反行为准则的行为可以通过 conduct@mxsec.io 报告。

## 许可证

本项目使用 [Apache License 2.0](../LICENSE)。所有贡献将在同一许可下发布。提交 PR 即表示你同意将你的贡献以 Apache 2.0 许可发布。
